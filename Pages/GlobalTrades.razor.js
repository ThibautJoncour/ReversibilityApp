...
export const handleFetchClick = (instanceReference) => {
    ...
    const tradeDateStart = $("#trade-date-start").val();
    const tradeDateEnd = $("#trade-date-end").val(); // DD-MM-YYYY
...
export function init() {
     ...
     */
    $(document).ready(() => {
        dataTable = $("#global-trades-table").DataTable({
            searchable: true,
            responsive: true,
            sortable: true,
            perPage: 10,
            ...
            labels: {
                ...
                info: "Showing {start} to {end} of {rows} trades"
            }
        });
    });
}